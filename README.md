...
The problem with PCRD and Mark-recapture models for Maximum likelihood estimation is that these models are very parameter hungry, often 20 -30 parameters for a simple fixed-effect model. One is often in a situation of <i>sparse data</i>, <i> low-sample sizes</i>, and <i>over-parametrization</i>. Symptoms include boundary-level estimates (e.g., MLEs at 100% survival or 100% detectability) with meaningless estimates. A 100% survival estimate is just plain stupid, despite being the Maximum Likelihood. Subjective Bayesian models (i.e., <i>not</i> using 'reference priors') help smooth over these boundary-level estimates. e.g., a MLE of p=1 will be pulled towards the prior expectation. Therefore, Bayesian models yield exact inference under any sample size. Of course, <i>here is the catch</i>: the prior plays a larger role under low-evidence/low-sample size situations. 

But there is evidence from the Machine Learning <i>predictive</i> analytics community, that such weakly informative priors yeild better long-term predictions than MLEs alone. This is the problem with Frequentist methods: we are pretending our experiments are done in a vacuum, alone in the universe, without insight from long-term study and predictive assments. In this way, Subjective Bayesian models share something in common with "Regularization" employed in the Machine Learning community, a view that is explored in [Hooten and Hobbs 2015](http://www.esajournals.org/doi/abs/10.1890/14-0661.1).

<b> BAYESIAN RESOURCES </b>

Here is an accessible presentation [Intro to Bayesian Inference and JAGS](https://drive.google.com/file/d/0BxeoeRy1g2jud3hjR0UwREV4Vk0/view?usp=sharing)(Murdoch University R User Group, 2015). Marc Kery's [WinBUGS for Ecologists](http://www.sciencedirect.com/science/book/9780123786050) is perhaps the best applied Bayesian books for non-statisticians, including R and BUGS code. [Cressie et al (2009)](http://www.esajournals.org/doi/abs/10.1890/07-0744.1) and [Halstead et al (2012)](http://onlinelibrary.wiley.com/doi/10.1111/j.1469-1795.2012.00540.x/abstract) are two great articles motivating Bayesian inference for ecologists. [Schofield and Barker (2001)](http://link.springer.com/article/10.1007/s13253-010-0052-4) and [Schofield et al (2009)](http://link.springer.com/article/10.1007%2Fs10651-007-0069-1) are accessible articles on mark-recapture generally in a Bayesian context.  Finally, Michael Jordon has a great video on ["Are You are Frequentist or Bayesian?"](http://videolectures.net/mlss09uk_jordan_bfway/).
